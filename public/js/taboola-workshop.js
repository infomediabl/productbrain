/**
 * Taboola Campaign Cloner — Standalone Page
 * Page: taboola-workshop.html
 * Globals used: none (self-contained)
 * API: GET /api/containers/:id, POST /api/containers/:id/taboola-campaign,
 *      GET /api/containers/:id/taboola-campaign/:campaignId
 *
 * Full Taboola campaign cloner UI: previous campaigns list, ad selector,
 * campaign settings, and launch flow.
 */
...
async function init() {
  if (!containerId) {
    document.getElementById('taboola-list').innerHTML = '<div style="color:var(--error);padding:16px;">No container ID. Go back to the dashboard.</div>';
    return;
  }

  const backLink = document.getElementById('back-link');
  if (backLink) backLink.href = `/container.html?id=${containerId}`;
  ...
  try {
    ...
    document.getElementById('taboola-list').innerHTML = `<div style="color:var(--error);padding:16px;">Error: ${esc(err.message)}</div>`;
  }
}
  ...
  selectorEl.innerHTML = `
    <div style="margin-bottom:12px;">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
        ...
        <button class="btn btn-ghost btn-sm" onclick="selectTopAds()" title="Select proven (non-new) ads">Select Top Ads</button>
        ...
        ${allAds.map(ad => {
          ...
          return `
            <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid var(--border);cursor:pointer;font-size:13px;" onmouseenter="this.style.background='var(--surface2)'" onmouseleave="this.style.background=''">
              <input type="checkbox" class="taboola-ad-cb" value="${esc(ad.id)}" onchange="updateCount()" style="accent-color:var(--primary);flex-shrink:0;">
              ${thumb}
    ...
    <button class="btn btn-primary" id="taboola-launch-btn" onclick="launchCampaign()" disabled>
      Preview Copy
    </button>
...
async function launchCampaign() {
  ...
  try {
    ...
    renderCampaigns();
    pollCampaign(data.taboola_campaign_id);
  } catch (e) {
...
async function pollCampaign(campaignId) {
  try {
    ...
    setTimeout(() => pollCampaign(campaignId), 3000);
  } catch (e) {
    setTimeout(() => pollCampaign(campaignId), 5000);
  }
}
